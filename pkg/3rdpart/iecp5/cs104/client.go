...
type Client struct {
	...
	onActive         func(c *Client)
	onUnActive       func(c *Client)
}
...
func NewClient(handler ClientHandlerInterface, o *ClientOption) *Client {
	return &Client{
		...
		onActive:         func(*Client) {},
		onUnActive:       func(*Client) {},
	}
}
...
func (sf *Client) SetActiveHandler(f func(c *Client)) *Client {
	if f != nil {
		sf.onActive = f
	}
	return sf
...
func (sf *Client) SetUnActiveHandler(f func(c *Client)) *Client {
	if f != nil {
		sf.onUnActive = f
	}
	return sf
...
func (sf *Client) clientHandler(asduPack *asdu.ASDU) error {
	...
	// 传输原因是20则回调到总召唤
	if asduPack.Coa.Cause == asdu.InterrogatedByStation {
		return sf.handler.InterrogationHandler(sf, asduPack)
	}

	// 传输原因是37则回调到电量总召唤
	if asduPack.Coa.Cause == 37 {
